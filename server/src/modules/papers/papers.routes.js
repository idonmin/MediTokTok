...
function applyFilters(query, params) {
  ...
  const searchMode = String(
    params.searchMode || (params.pmid ? "pmid" : "text"),
  ).toLowerCase();
  const searchTerm = String(
    params.searchTerm || params.query || params.pmid || "",
  ).trim();
  ...
  if (params.startYear)
    filtered = filtered.gte(
      "pubmed_records.pub_year",
      Number(params.startYear),
    );
  if (params.endYear)
    filtered = filtered.lte("pubmed_records.pub_year", Number(params.endYear));
  if (params.journal)
    filtered = filtered.ilike("pubmed_records.journal", `%${params.journal}%`);
  return filtered;
}
...
papersRouter.get("/", async (req, res, next) => {
  try {
    ...
    let query = database
      ...
      .select(
        "collected_at,pubmed_records!inner(pmid,pub_year,title,abstract,journal,authors)",
        { count: "exact" },
      )
...
papersRouter.get("/export", async (req, res, next) => {
  try {
    ...
    const csv = [
      columns.join(","),
      ...flattenCollections(data).map((row) =>
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="pubmed_records.csv"',
    );
    res.send(`﻿${csv}`);
  } catch (error) {
    next(error);
